...
function showResultScreen(winner, player1Score, player2Score, target) {
  ...
  const user1Results = window.getUser1Results ? window.getUser1Results() : [];
  ...
  const user2Results = window.getUser2Results ? window.getUser2Results() : [];
...
function setupEventListeners() {
  ...
  if (resetTargetBtn) {
    resetTargetBtn.style.display = 'none'; // ボタン自体を非表示にする
    // イベントリスナーも削除
    // resetTargetBtn.addEventListener('click', ... ) は不要
  }
